...
class Reputation(HelperCog):
    ...
    async def rep(self, ctx: commands.Context[HelperBot], member: disnake.Member) -> None:
        ...
        await ctx.reply(
            embed=ctx.bot.generic_embed(
                ctx,
                f"Added +1 rep to **{str(member)}**. The user has {total_reps} reps now!",
    ...
    async def add(self, ctx: commands.Context[HelperBot], member: disnake.Member, reps: int):
        ...
        await ctx.reply(
            embed=ctx.bot.generic_embed(
                ctx,
                f"Added +{reps} rep to **{str(member)}**. The user has {total_reps} reps now!",
                color=disnake.Color.green(),
            )
        )
    ...
    async def remove(self, ctx: commands.Context[HelperBot], member: disnake.Member, reps: int) -> None:
        ...
        await ctx.reply(
            embed=ctx.bot.generic_embed(
                ctx,
                f"Removed {reps} rep of **{str(member)}**. The user has {total_reps} reps now!",
                color=disnake.Color.green(),
            )
        )
    ...
    async def info(self, ctx: commands.Context[HelperBot], user: disnake.User | None) -> None:
        ...
        data = await ctx.bot.pool.fetchval(  # type: ignore
            """
    ...
    async def top(self, ctx: commands.Context[HelperBot]) -> None:
        data: typing.List[typing.Any] = await ctx.bot.pool.fetch(  # type: ignore
            """
            SELECT user_id, reps FROM devconnect_reps 
            ORDER BY reps DESC
            """
        )
        if len(data) == 0:
        ...
        await ctx.reply(
            embed=ctx.bot.generic_embed(ctx, leaderboard, title="Reputations Leaderboard", color=disnake.Color.green())
        )
